...
export default function App() {
  ...
  const registerPush = async (userId) => {
    if (!userId) {
      console.log("registerPush skipped: missing userId");
      return;
    }
    if (!("serviceWorker" in navigator)) {
      console.log("registerPush skipped: serviceWorker unsupported");
      return;
    }
    if (!("PushManager" in window)) {
      console.log("registerPush skipped: PushManager unsupported");
      return;
    }
    if (!("Notification" in window)) {
      console.log("registerPush skipped: Notification unsupported");
      return;
    }

    try {
      console.log("🚀 registerPush gestartet", { userId });
      ...
      if (permission !== "granted") {
        console.log("registerPush abgebrochen: permission nicht granted");
        return;
      }

      let sub = await reg.pushManager.getSubscription();
      if (!sub) {
        console.log("⚡ keine Subscription vorhanden, erstelle neu...");
        sub = await reg.pushManager.subscribe({
          userVisibleOnly: true,
          applicationServerKey: urlBase64ToUint8Array(VAPID_PUBLIC_KEY),
        });
        console.log("✅ Subscription neu erstellt", sub);
      } else {
        console.log("♻️ Subscription bereits vorhanden", sub);
      }

      const pushToken = JSON.stringify(sub);
      console.log("💾 Speichere push_token in freiwillige...");
      const { error } = await supabase
        .from("freiwillige")
      ...
      if (error) {
        console.error("❌ Fehler beim Speichern des push_token", error);
        return;
      }

      console.log("✅ push_token gespeichert");
      setUser((prev) => {
        if (prev?.type !== "freiwilliger" || prev?.data?.id !== userId) return prev;
        return {
          ...prev,
          data: {
            ...prev.data,
  ...
  const loadStellen = async (
    ...
    const effectiveUmkreis =
      umkreis ??
      (user?.type === "freiwilliger" ? activeVolunteerUmkreis ?? user?.data?.umkreis : null) ??
      null;
    const effectiveGemeindeId =
      gemeinde_id ||
      user?.data?.effective_gemeinde_id ||
  ...
  const executeVolunteerRadiusSearch = async ({ nextPlz, nextUmkreis, fallbackToProfile = false } = {}) => {
    if (user?.type !== "freiwilliger") return;

    const normalizedPlz = (nextPlz ?? filterPlz ?? "").trim();
    const radius = Number(nextUmkreis ?? filterUmkreis ?? user?.data?.umkreis ?? 50);

    if (fallbackToProfile) {
      const profilePlz = user?.data?.plz || "";
      const profileRadius = Number(user?.data?.umkreis ?? 50);
      setFilterPlz(profilePlz);
      setFilterUmkreis(profileRadius);
      setActiveVolunteerPlz(profilePlz || null);
      setActiveVolunteerUmkreis(profileRadius);
      await loadStellen(gemeindeId, profilePlz || null, profileRadius);
      return;
    }

    if (/^[0-9]{5}$/.test(normalizedPlz)) {
      setActiveVolunteerPlz(normalizedPlz);
      setActiveVolunteerUmkreis(radius);
      await loadStellen(gemeindeId, normalizedPlz, radius);
      return;
    }

    const profilePlz = user?.data?.plz || null;
    const profileRadius = Number(user?.data?.umkreis ?? 50);
    setActiveVolunteerPlz(profilePlz);
    setActiveVolunteerUmkreis(radius);
    await loadStellen(gemeindeId, profilePlz, radius);
  };
  ...
  useEffect(() => {
    ...
    setFilterPlz((prev) => (prev && prev.trim().length > 0 ? prev : profilePlz));
    setFilterUmkreis((prev) => (Number.isFinite(Number(prev)) ? Number(prev) : profileRadius));
    setActiveVolunteerPlz((prev) => prev || profilePlz || null);
    setActiveVolunteerUmkreis((prev) => (prev ?? profileRadius));
  ...
  const autoArchivieren = async (vereinId) => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      if (session) {
        ...
        const orderedRoles = [
          ...(preferredRole && roleLoaders[preferredRole] ? [preferredRole] : []),
        ...
        for (const role of orderedRoles) {
          resolvedUser = await roleLoaders[role]();
          if (resolvedUser) break;
        }

        if (resolvedUser) {
          setStoredLastRole(resolvedUser.type);
          ...
          if (resolvedUser.type === "freiwilliger") {
            ...
            setActiveVolunteerUmkreis(profil.umkreis ?? 50);
            setFilterPlz(profil.plz || "");
            setFilterUmkreis(profil.umkreis ?? 50);
  ...
  const handleBestaetigen = async (bewId, erschienen) => {
    const { data: bew } = await supabase
      .from("bewerbungen")
      .select("id, freiwilliger_id, termin_id, stelle_id, status")
      .eq("id", bewId)
      .maybeSingle();

    if (!bew) {
      showToast("Bewerbung nicht gefunden.", "#E85C5C");
      return;
    }
    ...
    const { data: terminBewerbungen } = await supabase
      .from("bewerbungen")
      .select("id, status, bestaetigt, nicht_erschienen")
      .eq("termin_id", bew.termin_id);

    const terminIstVollstaendigBearbeitet = (terminBewerbungen || []).every((row) => {
      const status = String(row.status || "").toLowerCase();
      return (
        row.bestaetigt === true ||
        row.nicht_erschienen === true ||
        ["erschienen", "no_show", "storniert", "abgesagt", "cancelled", "canceled"].includes(status)
      );
    });
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  ...
                  >
                    ...
                    <input
                      ...
                      onKeyDown={async (e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                          await executeVolunteerRadiusSearch({ nextPlz: e.currentTarget.value, nextUmkreis: filterUmkreis });
                        }
                      }}
                      onBlur={async (e) => {
                        await executeVolunteerRadiusSearch({ nextPlz: e.currentTarget.value, nextUmkreis: filterUmkreis });
                    ...
                    {filterPlz && (
                      <span
                        onClick={async () => {
                          await executeVolunteerRadiusSearch({ fallbackToProfile: true });
                        }}
                        style={{
                          fontSize: 12,
                          color: "#8B7355",
                  ...
                  <select
                    ...
                    onChange={async (e) => {
                      ...
                      await executeVolunteerRadiusSearch({ nextPlz: filterPlz, nextUmkreis: nextRadius });
                    }}
                    style={{
                      padding: "8px 10px",
                      borderRadius: 10,
        ...
        <LoginScreen
          onLogin={(type, data, gid) => {
            ...
            if (type === "freiwilliger") {
              ...
              setActiveVolunteerUmkreis(data?.umkreis ?? 50);
              setFilterPlz(data?.plz || "");
              setFilterUmkreis(data?.umkreis ?? 50);
              loadFollows(data.id);
          ...
          onSave={async (stelleData, termineData) => {
            try {
              const cleanStelle = {
                ...
                aufwand: stelleData?.aufwand || null,
                ort: stelleData?.ort || stelleData?.standort || user?.data?.ort || null,
                plz: stelleData?.plz || user?.data?.plz || null,
                standort: stelleData?.standort || stelleData?.ort || null,
                ansprechpartner: stelleData?.ansprechpartner || null,
                kontakt_email: stelleData?.kontakt_email || user?.data?.kontakt_email || user?.data?.email || null,
                telefon: stelleData?.telefon || user?.data?.telefon || null,
                dringend: Boolean(stelleData?.dringend),
                archiviert: false,
                aufrufe: 0,
                wirkungsbereich: stelleData?.wirkungsbereich || null,
                zielgruppe: stelleData?.zielgruppe || null,
                standard_dauer_stunden:
                  stelleData?.standard_dauer_stunden !== undefined && stelleData?.standard_dauer_stunden !== ""
                    ? Number(stelleData.standard_dauer_stunden)
                    : null,
                impact_typ: stelleData?.impact_typ || null,
                impact_beschreibung: stelleData?.impact_beschreibung || null,
                published: stelleData?.published ?? true,
                archived: false,
                umkreis:
                  stelleData?.umkreis !== undefined && stelleData?.umkreis !== ""
                    ? Number(stelleData.umkreis)
                    : null,
              ...
              if (stelleError || !stelle) {
                console.error("Stelle fehlgeschlagen:", stelleError);
              ...
              const cleanTermine = (termineData || [])
                ...
                .map((t) => ({
                  ...
                  endzeit: t.endzeit || null,
                  freie_plaetze:
                    t?.freie_plaetze !== undefined && t?.freie_plaetze !== ""
                      ? Number(t.freie_plaetze)
                      : Number(t?.plaetze || 5),
                  gesamt_plaetze:
                    t?.gesamt_plaetze !== undefined && t?.gesamt_plaetze !== ""
                      ? Number(t.gesamt_plaetze)
                      : Number(t?.plaetze || 5),
                  abgesagt: false,
                  status: t?.status || "geplant",
                  dauer_stunden:
                    t?.dauer_stunden !== undefined && t?.dauer_stunden !== ""
                      ? Number(t.dauer_stunden)
                      : null,
                }));
              ...
              if (cleanTermine.length > 0) {
                ...
                if (termineError) {
                  console.error("Termine fehlgeschlagen:", termineError);
                  showToast("Stelle gespeichert, aber Termine nicht.", "#E85C5C");
                  return;
                }
              }

              const { error: notifyError } = await supabase.functions.invoke(
                "notify-new-stelle-followers",
                {
                  body: { stelle_id: stelle.id },
                }
              );

              if (notifyError) {
                console.error("notify-new-stelle-followers fehlgeschlagen:", notifyError);
              }
              ...
              console.error("Stelle speichern unerwartet fehlgeschlagen:", error);
              showToast("Fehler beim Speichern der Stelle.", "#E85C5C");
            }
          }}
        />

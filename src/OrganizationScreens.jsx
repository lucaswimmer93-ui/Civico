          ...
          <>
            ...
            {meineStellen.length === 0 ? (
              ...
              meineStellen.map((s) => {
                ...
                const wartelistenBadges = (s.termine || [])
                  .map((t) => ({
                    id: t.id,
                    datum: t.datum,
                    startzeit: t.startzeit,
                    count: Number(wartelistenByTermin?.[t.id] || 0),
                  }))
                  .filter((t) => t.count > 0)
                  .sort((a, b) => {
                    const aTs = new Date(`${a.datum || ""}T${a.startzeit || "00:00"}`).getTime();
                    const bTs = new Date(`${b.datum || ""}T${b.startzeit || "00:00"}`).getTime();
                    return aTs - bTs;
                  });
                return (
                  <div
                  ...
                  >
                    ...
                    >
                      ...
                      {wartelistenBadges.length > 0 && (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
                          {wartelistenBadges.map((t) => (
                            <span
                              key={t.id}
                              style={{
                                display: "inline-flex",
                            ...
                            >
                              📋 {formatDate(t.datum)}{t.startzeit ? ` · ${t.startzeit}` : ""} · {t.count} auf Warteliste
                            </span>
                          ))}
                        </div>
                      )}
